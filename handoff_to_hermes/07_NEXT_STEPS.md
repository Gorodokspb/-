...
- Смета 881 → кнопка «Создать проект» → проект 9 → redirect → in_progress → ссылка «Открыть проект».
- Проект создаётся со статусом «Черновик» (future note: возможно, должен быть «В работе»).

### Future note: статус проекта из approved сметы
- Сейчас проект создаётся со статусом «Черновик».
- Позже можно отдельно решить, должен ли проект из согласованной сметы создаваться сразу «В работе».
